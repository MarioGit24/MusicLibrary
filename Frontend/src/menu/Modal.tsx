  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (t === "artist") {
        ...
        await fetch(
          `http://localhost:8081/recordlabels/1/enroll-artist/${newArtist.id}`,
          { method: "PUT" }
        );
      } else if (t === "single") {
        if (!selectedArtistId)
          throw new Error("Du måste välja en artist för singeln!");
    ...
    >
      <Fade in={open}>
        <Box sx={style}>
          <Typography
            variant="h5"
            sx={{ color: "#9e79a1ff", mb: 2, fontWeight: "bold" }}
          >
            ...
            {(currentType === "album" || currentType === "ep") && (
              <Box>
                <TextField
                  ...
                  sx={inputStyle}
                />
                <Typography sx={{ color: "#8e7990e4", mt: 2, mb: 1 }}>
                  Tracklist:
                </Typography>
                {(currentType === "album"
                  ? albumFormData.songs
            ...
            {currentType !== "artist" && (
              <FormControl variant="filled" fullWidth sx={selectStyle} required>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setSelectedArtistId(e.target.value as string)
                  }
                >
                  {artists.map((a) => (
